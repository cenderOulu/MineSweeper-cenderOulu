...
if modifiers & sweeperlib.MOD_SHIFT:
    # somethinghappens
"""
"""this sweeperlib.py has been modified at 423,424"""
import pyglet
...
graphics = {
    ...
    "fg_group": pyglet.graphics.Group(1),
    "text_group": pyglet.graphics.Group(2),
    "sprites": [],
    "images": {}
}
...
def create_window(width=800, height=600, bg_color=(240, 240, 240, 255), bg_image=None):
    """
    ...
    if graphics["window"] is None:
        ...
        graphics["window"].push_handlers(state["keys"])
...
def prepare_rectangle(x, y, width, height, color):
    ...
    graphics["sprites"].append(pyglet.shapes.Rectangle(
        x, y, width, height,
        color=color,
        batch=graphics["batch"],
        group=graphics["fg_group"]
